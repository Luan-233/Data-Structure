...
+ 线段树维护单调栈
  ...
  + 例题1：洛谷P4198 楼房重建
    ...
    + 当前结点为非叶子结点，首先检查左子结点中的最大值，若最大值小于等于$K$，则直接递归右子结点；否则左子结点中存在一部分信息保留，当前区间构成的单调栈中来自于右子结点的部分可以完全保留。

    ```c++
    inline int pushup(int L, int R, int cur, int k) {
...
+ 一些进阶分块题目：
  ...
  + 例题3：洛谷P4117 [Ynoi2018] 五彩斑斓的世界
    ...
    + $x\le V-x$，那么要在$O(x)$的复杂度内让区间的极差减小$x$，不能枚举大的部分就考虑枚举小的部分，这时候可以有等价操作：给序列中每个小于等于$x$的数加上$x$，然后查询$x$的出现次数的时候需要加上偏移量以后查询集合大小。
