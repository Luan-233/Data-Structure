...
编者注：这里的数据结构是一个广义的概念，只要是能够支持快速的利用性质存储、合并信息，以快速求解的算法都算入广义数据结构里面。这里面绝大多数是竞赛用数据结构与算法，少部分为工程应用里面的数据结构。所有的代码均为个人喜好写法。
...
+ 具体实现手法
  ...
  插入的时候只需要按照一般二叉查找树的插入手法实现即可。删除的时候采用惰性删除，即在要删除的结点上面打标记，在查找的时候只需要查找没有标记的一侧即可。
...
+ 区间最大子段和
  ...
  + 区间最大子段和的变式
    ...
    + 例题2：洛谷P2839 [国家集训队] middle
      ...
      我们考虑离散化以后二分答案，将$[a,d]$这一段区间内大于等于$mid$的权设为$1$，否则设为$-1$，如果$mid$能够成为某个子序列的中位数，那么这一段的数字和一定等于$0$或$1$，那么考虑当$mid$越来越大的时候，最大的权值和一定越来越小以致于小于$0$，在二分一个值的时候，按照上述说法处理这一段序列，求出左端点在$[a,b]$，右端点在$[c,d]$的最大子段和，若大于等于$0$则上调二分下界，这个正负$1$的序列可以通过权值递降的顺序建可持久化线段树求得。
...
后缀数组求解出来以后可以快速得出一个字符串的任意两个后缀的LCP。首先介绍$height$，即
