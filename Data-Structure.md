...
另一个技巧是多次区间加，然后最终询问一个静态的序列。通过差分的技巧可以做到线性。具体而言，设原序列$a_i$的差分序列$b_i=a_i-a_{i-1}$，每一次区间加后，假设对$[l,r]$增加$x$，那么差分序列中，改变的只有$b_l$与$b_{r+1}$，其中$b_l$增加$x$，$b_{r+1}$减去$x$，多次的区间加操作只是在差分序列中修改恰好两个数。最后通过对差分序列做前缀和还原回原序列。
