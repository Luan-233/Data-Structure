...
+ 堆实现贪心优化例题
  ...
  + 例题5：洛谷P4331 [BalticOI 2004]Sequence 数字序列
    ...
    下面来证明，$f(i,j)$与$g(i,j)$都是一个至多分$i+1$段的分段线性函数，并且下凸。进一步的，由归纳法可以证明，$f(i,j)$最后一段分段的斜率是$0$，$g(i,j)$存在最小值点。
...
容易发现，在出现结点不平衡的时候，很容易想到用旋转的方式来维护平衡。但是只旋转一次是否可行呢？我们分情况讨论一下。

由于任何一个结点均满足左右子树高度差的绝对值之差为$1$，所以插入或者删除一个结点以后，左右子树高度差至多变化$1$。所以在左右高度差为$2$的时候，就应当通过旋转调整。

考虑如下情况：假定左子树高度减右子树高度恰好为$2$，显然必要的一步是将当前结点右旋，以下设$height(cur)$表示结点$cur$的高度，讨论左子树的两个子树高度差之间的关系：

1. 1
1. 2

下面考虑删除，若删除的是叶结点，那么会很好维护，即一路旋转上去维护即可。若左右子树至多存在一个，那么直接取过来拼接即可。容易发现这两种情况下父结点左右高度差至多变化$1$，能够用类似于插入的方式进行维护。当左右子树均存在的时候，可以考虑用左子树里面最大值，或者右子树里面的最小值进行替换，因为左子树中的最大值就是左子树右脊柱的端点，一定没有右子树，所以转化为了上面至多只有一个子树的问题。同理右子树最小值。要注意的是，子树删除以后也要维护旋转。

那么只需要证明，每次将子树调整完以后，父结点左右高度差变化量至多变化$1$，这保证了每一步维护以后，每个点为根的树都是平衡的AVL树。



下面给出一个写法，不同于上面的Treap，每个结点会记录重复元素。实现的方式大同小异。

```c++
class AVL{
private:
    int n, m, ans, lastans;
    int root, ls[maxn], rs[maxn], height[maxn], val[maxn], siz[maxn], num[maxn], cnt;
    inline void init() {
    	ls[0] = rs[0] = siz[0] = height[0] = 0;
    }
    inline void pushup(int cur) {
    	siz[cur] = siz[ls[cur]] + siz[rs[cur]] + num[cur];
    	height[cur] = std::max(height[ls[cur]], height[rs[cur]]) + 1;
    }
    inline int newnode(int x) {
    	int cur = ++cnt;
    	val[cur] = x, siz[cur] = 1, num[cur] = 1, height[cur] = 1;
    	return cur;
    }
    inline void rotatel(int& cur) {
    	int tmp = rs[cur];
    ...
    inline void maintain(int& cur) {
    	if (height[ls[cur]] - height[rs[cur]] >= 2) {
    		int lson = ls[cur];
    		if (height[ls[lson]] >= height[rs[lson]]) rotater(cur);
    		else rotatel(ls[cur]), rotater(cur);
    	}
    	else if (height[rs[cur]] - height[ls[cur]] >= 2) {
    		int rson = rs[cur];
    		if (height[ls[rson]] <= height[rs[rson]]) rotatel(cur);
    		else rotater(rs[cur]), rotatel(cur);
    	}
    }
    pair<int, int> remove_first(int& cur) {
    	pair<int, int> res;
    	if (ls[cur]) {
    		res = remove_first(ls[cur]);
    		pushup(cur), maintain(cur);
    	}
    	else {
    		res = make_pair(val[cur], num[cur]);
    		cur = rs[cur];
    	}
    	return res;
    }
public:
    void remove(int& cur, int x) {
    	...
    	if (x < val[cur]) remove(ls[cur], x);
    		...
    		if (num[cur] > 1) --num[cur], --siz[cur];
    		else {
    			if (!(ls[cur] | rs[cur])) cur = 0;
    			else if (!ls[cur]) cur = rs[cur];
    			else if (!rs[cur]) cur = ls[cur];
    			else {
    				pair<int, int> tmp = remove_first(rs[cur]);
    				val[cur] = tmp.first;
    				num[cur] = tmp.second;
    				pushup(cur), maintain(cur);
    			}
    	...
    	else remove(rs[cur], x);
    	pushup(cur), maintain(cur);
    }
    void insert(int& cur, int x) {
    	if (!cur) { cur = newnode(x); return; }
    	if (x < val[cur]) insert(ls[cur], x);
    	else if (x == val[cur]) { ++num[cur], ++siz[cur]; return; }
    	else insert(rs[cur], x);
    	pushup(cur), maintain(cur);
    }
    int query_val(int cur, int rank) {
    	int offset = siz[ls[cur]] + num[cur];
    	if (rank <= siz[ls[cur]]) return query_val(ls[cur], rank);
    	else if (rank > offset) return query_val(rs[cur], rank - offset);
    	else return val[cur];
    }
    ...
    inline int lower_bound(int x) { return query_val(root, query_rank(root, x)); }
    inline int upper_bound(int x) { return query_val(root, query_rank(root, x + 1) + 1); }
};
```
...
#### 前缀和与差分-Suffix Sum & Difference

前缀和往往适合与差分混合使用，适用于静态序列并且存在运算存在逆运算的操作。

这种技巧最多见于区间求和，例如给定静态序列$a_i$，多次给定$[l,r]$，求$\sum_{i=l}^{r}a_i$，显然可以转化为$\sum_{i=1}^{r}a_i-\sum_{i=1}^{l-1}a_i$，那么设$Sum_n$表示$\sum_{i=1}^{n}a_i$，那么上面的区间和就是$Sum_r-Sum_{l-1}$，而$Sum_i$可以快速线性预处理出来。同样适用的有区间$xor$，区间矩阵乘法（前提是矩阵可逆）等。

另一个技巧是多次区间加，然后最终询问一个静态的序列。通过差分的技巧可以做到线性。具体而言，设原序列$a_i$的差分序列$b_i=a_i-a_{i-1}$，特殊的，$b_1=a_1$。每一次区间加后，假设对$[l,r]$增加$x$，那么差分序列中，改变的只有$b_l$与$b_{r+1}$，其中$b_l$增加$x$，$b_{r+1}$减去$x$，多次的区间加操作只是在差分序列中修改恰好两个数。最后通过对差分序列做前缀和还原回原序列。

一些基础的例题，用到了基础的前缀和与差分以及扩展思路：

+ 例题1：洛谷P1083 [NOIP2012 提高组] 借教室

  将问题转化为序列每次区间减一个数，问最多多少次以后序列仍然非负。容易发现，当某一天不合法以后，后续也会不合法。那么考虑二分天数，快速检查多次区间减以后是否有负数，差分加上前缀和即可。
+ 例题2：洛谷P3932 浮游大陆的68号岛

  首先设$i$存储$val_i$的物品，可以写出答案为$\sum_{i=l}^{r} val_i \times dist(i,x)$，并且由于序列是线性的，$dist$可以很方便地用前缀和维护。设$d_i$表示储物点$i$到储物点$1$之间的距离，那么$dist(x,y)=|d_x-d_y|$。

  一般来讲带有绝对值的可以通过讨论大小关系拆开，由于距离非负，当$x<y$的时候$dist(x,y)=d_y-d_x$，反之亦然。那么上式可以转化为$\sum_{i=min\{x,l\}}^{min\{x,r\}-1} val_i\times (d_x-d_i)+\sum_{i=max\{x,l\}+1}^{max\{x,r\}} val_i\times (d_i-d_x)$，再拆开可以发现只需要维护$\sum val_i\times d_i$与$\sum val_i$。拆式子的部分不做赘述。
+ 例题3：洛谷P4231 三步必杀

  考虑区间加等差数列的本质是什么，假设在区间$[l,r]$增加一个首项为$d$，公差为$s$的等差数列，在差分数组上则表现为$b_l$增加$d$，$b_{r+1}$减去$d+(r-l)\times s$，位于$[l+1,r]$的$b_i$增加$d$，即在差分序列上做两个单点修改和一个区间加法。所以在区间加法这一步，可以对差分序列再次差分，通过两次差分维护好所有的区间加法，在这之后再做单点修改得到差分数列，最后做前缀和还原出最终数列。
+ 例题4：洛谷P3745 [六省联考 2017] 期末考试

  考虑在已知出成绩时间的条件下，快速求出不愉快度。设当前时间为$T$：

  + 学生等待产生的不愉快度就是查询$\sum_{t_i<T}(T-t_i)$，可以转化为多少$T\sum_{t_i<T} 1$，以及$\sum_{t_i<T}t_i$，通过$O(n)$的预处理处理出前缀和，即记一个$sum_1(x)$表示$\sum_{t_i\le x} 1$，再记一个$sum_2(x)$表示$\sum_{t_i\le x} t_i$实现。当$T$超过$t_i$最大值的时候，$sum_2(T)=sum_2(max_{i=1}^{n}t_i)$，这部分可以$O(n)$线性预处理，$O(1)$查询得到。
  + 教师分配产生的不愉快度可以分类讨论，容易发现若$b_i>T$则必须要花费一定的代价把$b_i$变成$T$，考虑用哪一种方案合理：
    + 若$B\le A$，则直接用$B\sum_{b_i>T}(b_i-T)$的代价即可。因为无论怎样，$b_i>T$的部分一定要移除$b_i-T$，若使用$A$的代价移动，不仅代价不优，而且还有一个潜在的问题是当$\sum_{bi>T}(b_i-T)>\sum_{bi<T}(T-b_i)$的时候，多出的部分无法全部移动到$b_i<T$的部分上面，还需要再用一部分$B$的代价移除，显然在$B\le A$的时候不优。
    + 若$B>A$，则判断$\sum_{bi>T}(b_i-T)$和$\sum_{bi<T}(T-b_i)$的大小关系，若$\sum_{bi>T}(b_i-T)\le \sum_{bi<T}(T-b_i)$，说明多余的部分可以全部移动到不足的部分，那么使用$A$的代价移动即可。否则，要先用$A$的代价移动$\sum_{b_i<T}(T-b_i)$，剩余无法移动的$\sum_{bi>T}(b_i-T)-\sum_{bi<T}(T-b_i)$使用$B$的代价移除。

  容易发现，在$T\ge max_{i=1}^{n}b_i$的时候，教师分配产生的不愉快度不再增加，但是学生等待产生的不愉快度却随$T$增加而增长，说明$T > max_{i=1}^{n}b_i$的枚举已经不优，所以直接从$1$到$max_{i=1}^{n}b_i$枚举$T$计算一个最小的值即可。
  
  所以总的时间复杂度为$O(max_{i=1}^{n}t_i+max_{i=1}^{m}b_i)$。

---

#### 序列分治-Divide and Conquer

不那么常见的技巧，但是往往套路性强，以下皆用例题的形式讲解。

+ 涉及区间最小值最大值信息的分治
  + 例题1：P4755 Beautiful Pair
  + 例题2：P6406 [COCI2014-2015#2] Norma
+ 单调性分治
  + 例题1：
...
+ 基本操作
  ...
  + 单点加法，区间求和
    ...
    对于单点修改，假设给位置$x$的数加上$val$，那么需要找到所有包含$x$的区间维护一下即可。下面给出这个做法：首先给$t[x]$加$val$，然后给$x$加上$lowbit(x)$，如此迭代直到$x>n$。

    对于其正确性，容易发现这个过程是在不断地寻找下一个最小的$x'$，使得$x'$的区间包含初始位置。又因为树状数组上区间的性质，故这个过程也可以理解为，不断跳树状数组抽象出的树，从叶结点一直修改到根。对于当前跳到的位置$x$，若增量小于$lowbit(x)$，那么会发现$x'$所管辖的区间长度也小于等于增量，故此时区间不相交，不需要加以维护。当增量恰好等于$lowbit(x)$的时候，$lowbit(x+lowbit(x))\ge 2lowbit(x)$，故此时$x+lowbit(x)$管辖范围包含了$x$的管辖范围，它也就是恰好下一个要维护的位置。

    容易看出，$x$加上$lowbit(x)$的过程就是每一次找到$x$二进制下最下面的一段$1$，将其赋为$0$以后把下一位增加$1$。其至多进行$log_2x$次操作就会把$x$变成$2$的整数次幂，然后再进行加法就会让$x$翻一倍，至多进行$log_2n$轮操作就会退出，故单点修改的复杂度也是$O(logn)$的。
  ...
  + 区间加法，单点查询
    ...
    这类问题的维护套路就是对原序列做差分，查分以后某个元素的实际值就是差分序列的前缀和，区间修改只会涉及到差分序列两个位置的变动，为此只需要用树状数组维护这个差分序列即可。具体参考上面的序列差分知识。
  ...
  + 区间加法，区间查询
    ...
    $$
    由于区间加法在差分以后的差分序列只需要修改两个位置，可知只需要维护$a[i]$的前缀和以及$(i-1)a[i]$的前缀和就可以进行快速修改与查询了。区间加法和区间求和只需要拆成前缀进行操作即可。

+ 权值树状数组的应用

  + 例题1：洛谷P1908 逆序对

    考虑把数字从左到右加入到序列的末尾，每次加入前，查询有前面多少个数大于最后一个数，即求出有多少逆序对二元组以当前位置为右端点，求和加起来就可以得到整个序列的逆序对数。

    对整个序列离散化，从左到右扫描序列，维护一个支持单点修改区间查询的数据结构，设当前加入序列末尾的数为$x$，那么答案会增加前面大于$x$的数的个数，然后把$x$插入，本质上是单点修改区间求和，树状数组维护即可。
  ...
  + 例题3：洛谷P3616 富金森林公园
    ...
    于是将所有的询问离线，高度离散化，默认$0$位置与$n+1$位置有两个高度为$0$的石头，设$ans(i)$表示水面高度为$i$时的答案，首先计算每个石头分立的答案，然后两两合并，容易发现这个放在$ans$数组上就是一个区间加单点询问的问题。同理修改，只需要把合并的过程反过来，修改完高度再合并回去。

  + 例题4：洛谷P3760 [TJOI2017]异或和
  ...
  + 例题5：洛谷P3312 [SDOI2014] 数表
  
    容易发现在$(i,j)$处数表的值其实就是有大小限制的$\sigma(gcd(i,j))$，$\sigma(i)$即$i$的因子和，首先化简式子：
    $$
    \sum_{i=1}^{n}\sum_{j=1}^{m}\sigma(gcd(i,j))=\sum_{d=1}^{min(n,m)}\sigma(d)\sum_{i=1}^{\lfloor\frac{n}{d} \rfloor} \sum_{j=1}^{\lfloor\frac{m}{d} \rfloor} [gcd(i,j)=1]\\
    =\sum_{d=1}^{min(n,m)}\sigma(d)\sum_{k=1}^{k\times d \le min(n,m)}\mu(k)\lfloor\frac{n}{kd}\rfloor\lfloor\frac{m}{kd}\rfloor\\
    =\sum_{T=1}^{min(n,m)}\lfloor\frac{n}{T}\rfloor\lfloor\frac{m}{T}\rfloor \sum_{d|T} \sigma(d) \mu(\frac{T}{d})
    $$
    若没有$\sigma(d)\le a$的限制，上面的式子就可以预处理出$f(T)=\sum_{d|T} \sigma(d) \mu(\frac{T}{d})$的前缀和然后整除分块计算，在有大小限制的情况下，预处理出所有的$\sigma(i)$然后按照大小排序，询问也同样的离线排序，每处理一个询问就把所有的小于等于当前询问的$\sigma(i)$更新入$f$，通过枚举倍数实现。做询问的时候就整除分块，然后每次分块都是查询$f$的区间和，用一个数据结构做到单点修改区间求和，树状数组维护即可。
  
    设值域最大为$N$，则复杂度为$O(Nlog^2N+q\sqrt{N}logN)$。
  
+ 树状数组维护扫描线

  扫描线并不只是用在计算几何中，作为一种具象的算法。在很多数据结构相关的题目中，都会用到类似的思想。其实扫描线的本质就是，在两个甚至多个维度上存在限制的时候，暴力枚举一维（即模拟扫描线的移动），用数据结构维护另外一维的信息。

  + 例题1：洛谷P1972 [SDOI2009] HH的项链

    询问离线，按照右端点为关键字存入链表，对于每个元素记录$pre(i)$表示$i$前面最后一个和$i$颜色相同的位置，没有就设为$0$，考虑一个暴力的实现，从左向右扫描，如果某种颜色是第一次出现则进行记录，否则不会产生贡献。以此维护一个$01$序列，考虑固定右端点，向左扫描时记录所有的第一次出现的位置，若这个位置是第一次出现则赋为$1$，否则为$0$，那么此时右端点为$r$，左端点为$x$的答案就是$x$到$r$的区间求和。考虑右端点向右移动一位以后这个$01$序列会发生哪些变动，显然只有$r+1$这个位置会变成第一次出现，那么上一次出现$r+1$这个位置的颜色就会从第一次出现变成第二次出现，赋成$0$即可，只需要将询问离线，用一个单点修改前缀求和的树状数组即可完成操作。
  ...
  + 例题3：洛谷P2154 [SDOI2009]虔诚的墓主人
    ...
    首先离散化横纵坐标，考虑暴力的枚举每一个空位，显然一个空位的贡献可以由上下左右的树的数量用组合数算出，考虑优化这个过程，直接枚举同一列树的间隙，那么此时上下部分的组合数已经固定，只需要知道每个间隙内位置左右的组合数贡献之和就可以了。那么使用一个满足单点修改，区间查询的树状数组，假设当前处理横坐标是$x$，那么树状数组内位置$i$维护$(x,i)$的左部和右部的组合数贡献。从左向右扫描每一列，枚举纵向间隙，查询横向组合数计算贡献，每处理完一列就更新横向组合数贡献，显然每个障碍点会恰好更新一次一个位置的贡献，组合数杨辉三角预处理即可。总复杂度$O(nlogn)$。

  + 例题4：洛谷P2163 [SHOI2007] 园丁的烦恼

    经典静态平面矩阵和，考虑将一个询问拆分成四个询问，即若询问$x\in[x_1,x_2]$，$y\in[y_1,y_2]$的矩阵和，可以拆分为左下角为$[1,1]$，右上角分别为$[x_1-1,y_1-1]$、$[x_2,y_1-1]$、$[x_1-1,y_2]$、$[x_2,y_2]$四个矩阵的和相加减，一个询问拆分为至多四个询问，后面可以转化为二维前缀和。

    离散化所有坐标，将所有的询问（按照拆分出的矩形的右上角）和平面上的点坐标一起离线排序，以横坐标为第一关键字，纵坐标为第二关键字升序排序，当询问与点的坐标一样的时候，把询问放在后面。维护一个数据结构，逐一处理每个询问或者点，若遇到的是点则在纵坐标为下标的位置$+1$，否则以纵坐标为下标查询前缀和。容易发现只需要树状数组即可实现。

    因为在扫描的过程中，若遇到是询问$[1,1]$到$[x_i,y_i]$的矩阵和，那么在查询的时候，已经加入的点要么满足横坐标小于$x_i$，要么满足横坐标等于$x_i$且纵坐标小于等于$y_i$，且通过数据结构约束了纵坐标的条件，保证了查询的不重不漏。

  + 例题5：洛谷P3221 [HNOI2012]双十字
...
+ 树状数组二分
  ...
  + 例题1：洛谷P3997 [SHOI2013]扇形面积并
    ...
    容易发现一个扇形可以用扫描线维护，强制规定$x$轴为起始扫描线，那么一个扇形能够拆成至多两个小扇形。扫描线随着角度逆时针旋转，两条扫描线之间的扇形堆叠使用一个树状数组维护，维护所有的扇形半径，那么二分寻找最大的那个半径，满足这个半径内的扇形被覆盖至少$k$次。本质就是权值树状数组单点修改查询第$k$大。使用树状数组二分即可实现扫描线的维护以及最大半径查询。

  + 例题2：洛谷P7602 [THUPC2021] 视频倍增期

    使用类似于扫描线的思路，暴力枚举每个位置，用一个数据结构维护时间轴，枚举位置的时候维护在当前位置产生贡献的区间加法，可以在原序列上差分实现，即$[l,r]$上的一个区间加法可以拆分为$l$时刻增加，$r+1$时刻减少，且由于每个点的值随着时间增长单调不降，所以可以直接单点询问某个时刻的值，然后在这个单调序列上二分即可。使用差分技巧求和，将树状数组二分稍作修改便可实现查找第一个大于等于$x$的功能。

  + 例题3：洛谷P3960 [NOIP2017 提高组] 列队
...
+ 可持久化（主席树）
  ...
  + 例题2：洛谷P3466 [POI2008] KLO-Building blocks
  ...
  + 例题5：洛谷P3567 [POI2014]KUR-Couriers

    + 解法1：考虑区间的绝对众数超过$\frac{r-l+1}{2}$，那么在已知区间的权值线段树的情况下，设权值中点为$mid$，那么$[L,mid]$或$[mid+1,R]$只要有一侧所有数的出现次数之和超过$\frac{r-l+1}{2}$，解就有可能存在在这个区间内。当走到叶子结点的时候就找到一个解，否则就无解。
    + 解法2：考虑对区间进行排序，区间绝对众数如果存在，那么必然出现在序列中位数的位置，否则很容易反证绝对众数可能不唯一。由于绝对众数的唯一性，求出中位数以后做一步检查即可。

  + 例题6：洛谷P3248 [HNOI2016]树
...
+ 线段树合并
  ...
  + 例题4：洛谷P8496 [NOI2022] 众数

    这个题目有四个操作，序列的末尾插入、删除，两个序列的合并都可以用链表方便的维护，集合的维护可以直接动态开点后线段树合并，最后一问的区间绝对众数就是洛谷P3567的做法。
...
+ 区间历史和
  ...
  这里采用的讲解是设一个所谓的操作队列，即对于一个结点，施加在上面的操作用一个连续的队列来实现，操作有两种，一个是对这个结点的对应区间做加法，另一个是更新历史和。在这样的结构下，维护一定的信息，我们就可以保证两个操作队列能够顺次合并，以此实现线段树下推标记的效果。那么我们需要维护的标记信息不仅有加法标记，还增加了加法标记历史和、施加的加法标记次数。下推的时候，应当更新区间和、历史和、加法标记。

  下推时，首先要更新历史和，首先历史和会相较于之前增加区间和乘上更新次数，再增加后续的标记之和。然后更新区间和。最后合并所有的标记信息。

  更具体的，我们设$hsum$表示区间历史和，$sum$表示区间和，一个结构体记录标记信息，其中$times$表示进行的历史和求和次数，$tag$表示最后一次的加法标记，$add$表示$times$次历史和更新时，每一次的$tag$之和。

  ```c++
  int sum[maxn << 1], hsum[maxn << 1];
  struct opt {
      int times, tag, add;
  } o[maxn << 1];
  inline void applay(int cur, int L, int R, opt x) {
      int len = R - L + 1;
      hsum[cur] += sum[cur] * x.times + x.add * len;
      sum[cur] += x.tag * len;
      o[cur].add += o[cur].tag * x.times + x.add;
      o[cur].times += x.times;
      o[cur].tag += x.tag;
  }
  ```

  如果执行区间加，那么就对线段树对应区间区间执行`applay(cur, L, R, opt{ 0,val,0 })`。

  若更新一次历史和，就全局执行`applay(root, 1, n, opt{ 1,0,0 })`。

  下面是一些例题：

  + 例题1：洛谷P8868 [NOIP2022] 比赛

    一个明显的思路，就是把询问离线做扫描线更新，每个位置维护$a$、$b$，以及维护$a\times b$的历史和。也即当处理到$x$时，线段树中每个位置$i$维护$\sum_{j=i}^x max\{a_i\dots a_j\}\times max\{b_i\dots b_j\}$，后缀最大值可以用单调栈线性维护，剩余的就是如何使用线段树配合标记维护上述信息。

    考虑把标记扩展到两个维度$a$与$b$，那么需要维护的信息有：历史和更新次数、$a$的标记、$b$的标记、$a$的标记和、$b$的标记和、$a\times b$的每一次的标记和。

    下推的时候同样考虑标记和信息相对于历史信息的变化，对于历史和，我们可以看做是每个版本增加偏移量后相乘求和，若用$\Delta a_i$表示第$i$次更新$a$的标记以后，$a$的标记值，也就是增量之和，可以拆式子拆开：
    $$
    \Delta hsum=\sum (a+\Delta a_i)(b+\Delta b_i)\\
    =ab\times times+a\sum\Delta a_i+b\sum\Delta b_i+\sum \Delta a_i\Delta b_i
    $$
    这个式子是单个位置的信息变化，转成区间信息就是$a$与$b$的区间和。标记维护的是单个位置，所以最后要乘上区间长度。

    对于标记信息的变化，推式子的方式和上面一样。

    ```c++
    typedef long long ll;
    ll sum[maxn << 1], suma[maxn << 1], sumb[maxn << 1], sumab[maxn << 1];
    struct opt {
    	ll times;
    	ll taga, tagb;
    	ll adda, addb, addab;
    ...
    inline void applay(int cur, int L, int R, opt x) {
    	ll len = R - L + 1;
    	hsum[cur] += sumab[cur] * x.times + suma[cur] * x.addb + sumb[cur] * x.adda + x.addab * len;
    	sumab[cur] += suma[cur] * x.tagb + sumb[cur] * x.taga + x.taga * x.tagb * len;
    	suma[cur] += len * x.taga;
    	sumb[cur] += len * x.tagb;
    	o[cur].times += x.times;
    	o[cur].addab += o[cur].taga * o[cur].tagb * x.times
            		+ x.addab + o[cur].taga * x.addb + o[cur].tagb * x.adda;
    	o[cur].adda += o[cur].taga * x.times + x.adda;
    	o[cur].addb += o[cur].tagb * x.times + x.addb;
    	o[cur].taga += x.taga;
    	o[cur].tagb += x.tagb;
    }
...
+ 扫描线的维护
  ...
  + 例题1：洛谷P5490 【模板】扫描线（矩形面积并）
    ...
    对于所有的矩形上下边界，记录其横坐标两个端点，纵坐标以及是否为下边界，自下向上做扫描线，若一条线段代表的是矩形下边界，就在区间数据结构内把这个区间加$1$，否则减$1$。在处理一个纵坐标之前，查询有多少位置满足非$0$，再乘上相邻两个不同的$y$之间的差值，非$0$位置个数可以用全长减去$0$的数量，又发现最小值永远不会小于$0$，那么可以特殊转化为维护最小值以及其出现次数即可。
  ...
  + 例题2：洛谷P1856 [IOI1998] [USACO5.5] 矩形周长Picture
    ...
    考虑沿用上面的做法，发现周长可以分解为水平线段长度加上竖直线段长度，并且两个维度的求解等价，于是以求解水平线段长度为例。考虑什么地方的线段会成为周长的一部分，对于一个固定要处理的$y$上面的线段的部分，沿用上面累加线段树的做法，其成为边长的条件是：处理完后一个位置由$0$变为非$0$，或者由非$0$变成$0$，前者对应矩形下边界成为边长的一部分，后者对应矩形上边界。处理时，对于固定的一个$y$，先把所有下边界加入线段树，查询一遍前后变化量得到由$0$变非$0$的位置个数，再把所有上边界移除，查询变化量得到由非$0$变$0$的位置个数，累加即可。同理竖直线段的求解。

  + 例题3：洛谷P8600 [蓝桥杯 2013 省 B] 连号区间数

    当一个区间$[l,r]$是满足排序以后是连续数值的时候，一定满足：$max-min+1=r-l+1$，同时要求序列中不能出现相同的数。由于题目中给定了是一个排列，故上面的式子就变成了充要条件。

    考虑枚举一个区间的右端点，相当于已知了$r$，上面的式子移项变成$max-min-r+l=0$，设原序列为$a_i$，对于$i\le r$维护$max_{j=i}^{r}a_i-min_{j=i}^{r}a_i-r+i$，又发现当序列中整数两两不同的时候，极差一定不小于序列长度，所以这个值非负，这意味着$0$是上式的最低可能取值，那么维护区间最小值以及出现次数即可。当$r$右移一位的时候，有关$min$与$max$的变化是一个区间，用单调栈维护，虽然本质是一个区间覆盖，但其实可以转化为多个分段的区间加法，每弹掉一个栈内元素就做一次区间加法推平，总的区间加次数是$O(n)$的。$r$的变化就是简单的区间加法。

  + 例题4：洛谷P3246 [HNOI2016] 序列

    首先考虑贡献的套路，设这个序列为$a$，考虑每一个位置能够成为多少个区间的最小值，对于相同元素，我们令一个子段的最小值是这一段的最小值中最靠左的。那么利用单调栈处理出左面第一个小于等于当前位置的值的位置，以及右面第一个小于当前值的位置。设位置$i$能够向左最多延伸到$left(i)$，向右最多延伸到$right(i)$。
...
+ 值域分块
  ...
  一般要套序列分块一起使用，用于求解区间第K大。一般用$cnt_1(i,j)$表示前$i$个块内$j$出现了多少次，$cnt_2(i,j)$表示前$i$个块内值域有多少数位于值域的第$j$个块内，然后零散部分（如果询问跨块，那么零散部分就是左右两端，否则就是询问的序列）记录$cnt_3(i)$为零散部分有多少$i$，$cnt_4(i)$为零散部分有多少数落入值域块$i$，然后从小到大枚举值域块，类似于线段树二分，如果定位了某个块就枚举这个值域块内部的元素，求解完成后撤销零散块贡献。

  + 例题1：洛谷P4119 [Ynoi2018] 未来日记

    首先按照上面说的进行序列与值域分块，然后每个块记录一个映射，$f_1(i)$表示当前块内$i$代表什么数，$f_2(i)$表示当前块内什么数代表$i$，然后零散块修改直接重构，记录块内有多少$x$，整块考虑三种情况：

    + 没有$x$，那么直接跳过；

    + 有$x$，没有$y$，直接用上面提到的$f_1$、$f_2$进行映射，同时记录块内有多少个$x$；
    + 有$x$，有$y$，直接重构这个块，记录块内有多少$x$。

    然后对于修改，记录每个块有多少$x$被修改，求一个前缀和后对$cnt_1$、$cnt_2$进行修改维护。
...
这里以单点修改，区间第$K$大为例，讲解整体二分的实现方法。（洛谷P2617 Dynamic Rankings）

将序列中的每个初始的数表示为单点插入，然后将询问与修改都放进一个大数组里一起操作。询问的形式就是求解一个区间的第$K$大，序列初始化的形式是单点增加一个数，修改的形式是单点移除一个数再加入另一个数。所有的操作从左到右按照时间顺序递增。预先开好一个允许单点加区间求和的数据结构，以及两个栈$stk_1$、$stk_2$用来分离所有的操作。

在我们已知所有的元素的取值范围以及所有的操作以后，设函数`solve(L,R,l,r)`表示当前处理所有的修改的值域以及询问的答案均位于区间$[L,R]$内，这一部分在所有的操作序列$[l,r]$中。

考虑二分法检查答案，即检查区间内有多少数小于等于$mid$。令$mid=\lfloor \frac{R+L}{2} \rfloor$，然后从$l$到$r$枚举操作，若此时对应的增加/移除操作，且这个操作的权值小于等于$mid$，就在一个序列内把这个位置的值增加/减少$1$，并且把这个修改放入$stk_1$，否则只需要放入$stk_2$；若遇到查询操作，就在序列中查询这一段有多少个$1$，也就是检查当前这一段内值域位于$[l,mid]$中的数有多少，设这个数值为$offset$，若$rank \le offset$则把这个询问放入$stk_1$，否则将$rank$减去$offset$，然后把询问放入$stk_2$。这个过程类似于线段树上二分，也就是查询左子树中的元素个数以决定向哪一个子结点迭代。处理完以后就把$stk_1$中的操作进行一遍撤销，将两个栈中的操作放回操作序列$[l,r]$，其中$stk_1$在左，$stk_2$在右，向下递归`solve(L,mid,l,l+top1-1)`和`solve(mid+1,R,l+top,r)`。当递归到某一段没有询问的时候就返回，当值域区间长度为$1$的时候，就可以直接更新对应操作区间内询问的答案了。
...
+ 可以优化至$O(n+m)$的rmq
  ...
  + 约束rmq的$O(n+m)$求解
    ...
    + 当询问跨块的时候，直接零散块查表以后大块rmq，考虑当询问落在一个块内的时候怎么处理，我们要充分利用差值的绝对值为正负$1$的条件，需要额外记录一下每个块中的最小值是多少，然后把这个块内的所有元素减去这个最小值，注意到块长很短，全做减法以后相邻差值不变，我们认为两个块不同当且仅当在减去块内最小值以后，块内存在若干元素不同。此时由于差分数组的取值只有$\pm 1$，本质不同的块至多有$2^{0.5log_2n}$种，这个是$O(\sqrt{n})$级别的，因此对这本质不同的$O(\sqrt{n})$种块跑$O(size^2)$的暴力处理结果，查询时直接映射即可。这部分处理复杂度亚线性，单次查询$O(1)$。查询完以后需要把减去的最小值加回来。

  + $O(n\alpha(n)+m)$的做法：

    根据笛卡尔树的性质（任意两个点的LCA处记录区间最值），可以线性建出笛卡尔树（见笛卡尔树部分），用Tarjan离线LCA的方式求出区间最值，主要时间复杂度集中在离线LCA的并查集上。
...
+ 树上莫队
  ...
  + 例题：
    ...
    + 例题1：SPOJ10707 COT2 - Count on a tree II
    
      就是把数颜色放到树上了。按照上述实现方式实现即可。
    
    + 例题2：洛谷P4074 [WC2013] 糖果公园
...
+ 维护偏序统计量：
  ...
  + 例题5：洛谷P5445 [APIO2019] 路灯

    考虑设一个平面，平面上$(x,y)$存储点对$x$到$y$从开始到当前时刻的答案（在这里强制规定$x<y$），那么整个问题可以转化为偏序问题。

    具体而言，当$xy$不连通的时候，在当前时刻$(x,y)$处的值就是所求的答案。但是在$xy$连通的时候，$(x,y)$存储上一次$xy$不连通时候的值加上当前时间减去上一次连通的时间。

    对于答案的求解，若$xy$不连通，$(x,y)$存储的就是从开始到当前时刻的答案，若$xy$连通，那么$(x,y)$存储的就是从开始到最后一次断开的答案减去最后一次连通的时间。同样的，当$xy$不连通，做单点查询即可得到解，当$xy$连通，答案是单点查询所得到的结果加上当前查询的时间点。

    容易发现上面这种形式的维护，在逐一做询问的时候$(x,y)$上存储的值不会发生变化，只有当修改的时候才会改变。具体而言，修改的时候需要求得对于一条连向$(i,i+1)$的边，求出最左的端点$L$和最右的端点$R$，使得$L$到$i$连通且$i+1$到$R$连通，此时会影响的点$(x,y)$满足$L\le x\le i$，$i+1\le y \le R$，针对两种情况讨论：若原先$(i,i+1)$连通，那么上面的$(x,y)$都会断开，其位置的值增加当前时间（存储从开始到当前时刻的答案）；若不连通，那么上面的$(x,y)$都会连通，其位置的值减去当前时间，也与上面的表述相对应。

    显然上面所做的是：矩阵加法，单点求值。可以用四次差分配合CDQ分治和树状数组解决，连通性的求解可以用树状数组配合二分答案解决。
...
+ DFS序
  ...
  + LOJ P146 DFS序3：链加，子树求和，单点查询
    ...
    考虑链加对单点查询的影响，仍考虑弱化问题，链加对链上一个点的贡献是其增加的值$val$，那么考虑转化为类似于树上差分的思路，改为单点加法子树求和，使用树状数组维护即可。
...
+ 轻重链剖分
  ...
  全局数组声明中，$top$代表一条重链的顶端结点编号，$fa$记录父节点，$siz$记录子树大小，$deep$记录结点深度，$son$表示重儿子，$dfn$为DFS序。
...
+ 长链剖分
  ...
  类似于重链剖分，惟一的区别是记录的儿子指向的是向下能够走出路径最长的一个子节点，类似的可以分析出每个结点到根结点的链切换次数都是$O(\sqrt{n})$的。

  长链剖分可以用于维护形如“子树内与当前点距离为$d$”的信息的快速统计。其本质上是复用数组。设一个结点为$cur$，其一个子结点为$son$，设$f(i,j)$表示$i$子树内与$i$距离（两点之间的边数）为$j$的结点信息，那么$f(son,i)$一定是$f(cur,i+1)$的子集。所以考虑复用信息，用一个长为$n$的数组记录一条结点数为$n$的链的信息，这里可以使用一些指针上的技巧进行读写，具体实现可以见下。
  
  在做信息合并的时候，长链可以用来将有关深度信息的求解优化至$O(n)$，类似于轻重链剖分分出轻儿子与重儿子，每个点也需要记录一个“长儿子”与”短儿子“，容易发现长儿子子树内最深的点大于等于任何一个短儿子。所以在做深度信息合并的时候，长链剖分可以等价的看做是把一条链合并向另一条链，所以每个点恰好会在合并时被枚举到一次，可以优化复杂度至线性。
  ...
  + 深度信息的维护
    ...
    + 例题1：洛谷P6074 最小路径
      ...
      这是一个典型的01分数规划的形式，于是二分答案，检查是否存在一条路径满足$\frac{\sum a_i}{\sum b_i} \le mid$，转化成$\sum a_i-mid\times b_i<0$，设一个点的点权为$a_i-mid\times b_i$，也就是找一条权值最小的长度恰好为$m$的路径。
  
      长链剖分，预处理出每个点到链顶的权值之和，长链合并的时候需要给子树的长链值增加当前点到链顶的权值和更新，由于记录的信息还包括父结点到链顶的权值和，所以更新答案的时候需要额外减掉。
  
    + 例题2：洛谷P3899 [湖南集训] 谈笑风生
  
      考虑三元组内$b$与$a$的相对关系，若$b$是$a$的祖先结点，那么这部分的答案易于处理。当$a$是$b$的祖先结点的时候，考虑将询问离线以后长链剖分，设计状态$f(i,j)$表示以$i$为根的子树内，与$i$距离为$j$的结点的子树和（子树和不包含自己），这部分满足$p=i$的答案就是$\sum_{d=1}^{k}f(i,d)$，用一个树状数组按照长链剖分的DFS序维护合并和查询即可。
    
  + 树形DP优化
  
    + 例题1：洛谷P5904 [POI2014] HOT-Hotels 加强版
    
      考虑三元组的形态，容易发现合法的$(x,y,z)$一定满足存在一个点$d$，使得在以$d$为根的时候，$x,y,z$分属于不同的子树且深度相同。一个常用的方法就是处理出两个点的情况，然后枚举第三个点进行拼接。
    
      设$f(i,j)$表示以$i$为根的子树内，距离$i$为$j$的点有多少个。这个易于用长链剖分处理。
    
      为了便于枚举第三个端点进行拼接，设$g(i,j)$表示对于$i$子树内的所有点$x$，$x$的两个不同子树内有点$y$与$z$，满足$dist(x,y)=dist(x,z)=dist(i,x)+j$的所有满足条件的$(x,y,z)$的方案数之和。容易发现在已知$g$的条件下，可以方便的枚举第三个端点进行合并。
    
      仔细观察发现有用的$g$一定满足其$j$这一维不超过其向下走的最长距离，可以用长链剖分优化。并且注意到$g(i,j)$由$g(son,j+1)$继承而来，所以$g$数组要倒向申请，且长度为链长的$2$倍。
    
      考虑合并，DFS时枚举第三个端点更新答案，包含的情况有：以当前点为第三个端点（最后处理即可）、第三个端点在新子树内、第三个端点在枚举过的子树内，然后考虑合并：$g(i,j)$由$g(son,j+1)$和$f(son,j-1)\times f(i,j)$合并而来，即考虑$g$的中转点在子树内还是当前处理的点。$f$易于处理。
    
      给出一段核心代码，即DFS计算其贡献的部分：
      ...
      void dfs(int pos) {
          ...
          for (int i = head[pos]; i; i = nxt[i]) {
              if ((to[i] == fa[pos]) || (to[i] == son[pos])) continue;
              dfs(to[i]);
              for (int k = 2; k < len[to[i]]; ++k) ans += g[to[i]][k] * f[pos][k - 1];
              for (int k = 0; k < len[to[i]]; ++k) ans += f[to[i]][k] * g[pos][k + 1];
              for (int k = 1; k < len[to[i]]; ++k) g[pos][k - 1] += g[to[i]][k];
              for (int k = 0; k < len[to[i]]; ++k) g[pos][k + 1] += f[to[i]][k] * f[pos][k + 1];
              for (int k = 0; k < len[to[i]]; ++k) f[pos][k + 1] += f[to[i]][k];
          }
          ans += g[pos][0];
      }
...
+ 基于DFS序的换根操作
  ...
  + 例题3：洛谷P3676 小清新数据结构题
    ...
    + 对于点$x$，显然变化为全局和的平方。也就是原先$1$号点的子树和平方。
    + 对于从$fa(x)$到$1$路径上的点$y$，其权值从$sum(y)^2$变化为$(sum(1)-sum(z))^2$，其中$z$满足$fa(z)=y$且$LCA(x,z)=z$。那么考虑每一个路径上的点在和式中的出现次数，写出变化量和式即为$\sum (sum(1)-sum(z))^2-sum(y)^2=(deep(x)-deep(1)-1)sum(x)^2-2sum(1)\sum sum(cur)$。其中$\sum sum(cur)$表示从$x$到$1$且不包含$1$的路径上的点的子树和的和。
...
+ 点分治
  ...
  + Part2：考虑怎样确定中心位置$x$使得经过$x$的路径数目尽可能的多。
    ...
    在重心性质的加持下，点分治复杂度为$T(n)=O(n)/O(nlogn)+k\times T(\lfloor \frac{n}{k} \rfloor)=O(nlogn)/O(nlog^2n)$。$k$是一个随树形态不同而变化的常数。容易发现$k$越大，分成的子树越多，每个子问题越小，复杂度越低。
...
+ 例题3：P4062 [Code+#1] Yazid 的新生舞会

  将数字按照出现次数分类，以阈值$B$为大小分为两类：

  当出现次数小于等于$B$的时候，由区间绝对众数的性质知，若一个区间绝对众数出现次数是$num$，那么这个区间的长度一定不会超过$2num-1$。所以对于出现次数小于等于$B$的数字，只需要暴力枚举所有长度不超过$2B-1$的区间，记录众数的出现次数更新答案即可。复杂度$O(nB)$。

  当出现次数大于$B$的时候，可以对这些数逐一处理，设当前处理的数字是$x$，考虑设$[1,i]$的前缀内$x$的出现次数是$sum_i$，那么一个区间$[l,r]$满足条件，需要满足$sum_r-sum_{l-1}>\frac{r-l+1}{2}$，转化成$2sum_r-r>2sum_{l-1}-(l-1)$，是一个二维偏序的形式，套路地枚举右端点，用一个数据结构维护左端点的信息。但是容易发现当右端点右移一位的时候，$2sum_i-i$的变化量是常数级别的，也就是说可以暴力维护前缀和，记录上一次查询的右端点位置和答案，查询的时候暴力移动到当前位置即可。复杂度$O(\frac{n^2}{B})$。

  在$B=O(\sqrt{n})$的时候，复杂度为$O(n\sqrt{n})$。

+ 例题4：洛谷P5397 [Ynoi2018] 天降之物
...
势能可以理解为未来数据结构进行调整需要耗费的代价，任何时刻势能非负，数据结构进行额外调整所需要的代价全部从势能中扣除，只要我们在一次操作后合理增加势能，就可以保证未来开销的合理性。当然这个概念并不局限于数据结构，还有许多类似可以进行的分析。举一个现实中的例子，每天往银行里存入$1$块钱，然后时不时地从银行里把钱取出来，并保证没有欠债，那么取出来的钱不会超过存入钱的总量。几乎所有的数据结构都可以用类似的分析手段。

+ Sample1：二进制加法计数器递增的分析	

  大概意思就是，存在一个二进制数，每一次操作的含义为让这个二进制数加$1$，容易发现总的代价为$01$位的变化量和操作总量。

  定义这个二进制计数器的势能$\Phi(S)$为当前计数器内有多少位为$1$，容易发现二进制计数器的本质是，推平这个二进制数的最低位的连续一段$1$，然后把下一个$0$变成$1$。为方便分析，我们默认从$0$开始计数，那么以此为基础进行归纳：

  + 势能的增加手段为：每一次给计数器加$1$的操作让势能加$1$，然后在此基础上证明势能永远非负：
  + 初始状态下，计数器为$0$，势能为$0$，成立。
  + 在势能等于计数器中$1$的个数的前提下，每一次推平最低位的极长$1$段，这时候势能下降数值为这段$1$的长度，然后把下一位赋为$1$，势能增加$1$，增加的$1$对应于上述给计数器加$1$的时候顺带增加了势能。

  容易发现势能永远非负，进而知道势能下降总量不超过增加量，容易知道每一次操作中固定进行一次赋$1$，这个代价总量是$O(m)$的，然后势能总的增加量是$m$，那么总的推平$1$的代价不超过$m$，那么总的维护代价是$O(m)$，更严格来说，维护代价不超过$2m$，若感兴趣可进行自证。
...
最大子矩形即前面在单调栈版块所提到的最大子矩形问题，即在区域内给出若干“脏点”，找一个面积最大的子矩形，使得没有任何一个“脏点”在里面。

容易发现，最大子矩形一定是一个极大子矩形，也就是上下左右四个边界均达到边界，再向外扩展一点就会使区域增加“脏点”，那么我们可以考虑枚举一个“脏点”作为上方起点，从这个“脏点”出发向下，枚举下边界能够到达的位置，这就构成了所谓的悬线。在已知悬线上下边界的条件下，从悬线向左和向右能够求出最长的扩展距离，相乘就可以得到一个可能的候选解。

若所给问题的形式是二维网格图，以洛谷P4147 玉蟾宫为例，可以证明每个非“脏点”恰好位于一条悬线上，因为每个点向上能够恰好找到一个边界。也就是说悬线下边界的枚举其实会不重不漏的遍历所有的非“脏点”，这也就是它复杂度$O(nm)$的原因。

详细而言，对于每个格子记录$up(i,j)$表示从$(i,j)$向上能够延伸出的最长距离，$l(i,j)$与$r(i,j)$分别表示从$(i,j)$出发向左（或者向右）能够扩展出的最长长度，再记录$l_h(i,j)$与$r_h(i,j)$，其中$l_h(i,j)=min_{i'=up(i,j)}^{i} l(i,j)$，同理$r_h(i,j)$，有了这些信息，就可以很方便地枚举悬线的下边界，以及在矩形从这条悬线向左和向右能够延伸出的最大长度，进而求得最大子矩形。

给出一个滚动数组版本的实现。（谷P4147 玉蟾宫）

```c++
inline int solve() {
	int ans = 0;
	for (int i = 1; i <= m; ++i) lh[i] = rh[i] = m;
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            l[j] = (map[i][j] == 'F') ? (l[j - 1] + 1) : 0;
            lh[j] = min(lh[j], l[j]);
        }
        for (int j = m; j >= 1; --j) {
            r[j] = (map[i][j] == 'F') ? (r[j + 1] + 1) : 0;
            rh[j] = min(rh[j], r[j]);
        }
        for (int j = 1; j <= m; ++j) {
            if (map[i][j] == 'R') up[j] = 0, lh[j] = rh[j] = m;
            else if (map[i][j] == 'F') {
                ans = max(ans, (lh[j] + rh[j] - 1) * (++up[j]) * 3);
            }
        }
    }
    return ans;
...
另外，在“脏点”点集不分布在网格点上，而是分布在一般的二维平面上的时候，仍然可以沿用上面的思路，枚举一个点作为悬线的起点，从这个起点开始向下，自上而下枚举所有纵坐标小于起点的“脏点”。如果在悬线左面，就更新向左延伸出的最长距离，同理右侧的情况。

从上面的过程中可以发现，左右延伸出的最长距离（下面称之为矩形的宽度）不变的情况下，悬线一定会尽可能地向下延长直至宽度缩减。当宽度再向下一点就会缩减的时候，代表上方达到了一个极大子矩形，可以用当前的面积来更新答案。

这时候就要对所有的“脏点”按照纵坐标自上而下排序，枚举一个起点，对于每个起点自上而下枚举下侧“脏点”更新宽度，若“脏点”有$n$个，那么时间复杂度就是$O(n^2)$的。
...
其实数据结构一直不是一个很具体的概念，只要是计算机内，用于组织数据的逻辑结构被代码所实现出来，就是数据结构。

首先我想表达的是，数据结构是一种强有力的工具，它之所以能够优化求解，是因为数据结构能够依据数据的性质优化数据相关的处理查询操作。

更加重要的一点，绝大多数数据结构题目，没有唯一的、固定的数据结构可以成为它的唯一解。我所提供的思路，仅仅是提供一种尽可能优秀的实现（从代码难度、时间复杂度、空间复杂度等角度出发），譬如，单点修改区间求和的问题，我们当然可以选择线段树，你甚至可以选择平衡树去炫技，但就仅仅实现这个功能而言，树状数组无论是从时空还是代码难度上，都优于前面提到的两者。所以我在很多题目里面的总结往往不是一个固定的数据结构，而是一个抽象出来的、需要实现特定功能的数据结构，在这个基础之上，我们进行多方面的平衡，选择尽可能优的结果。就好像人生一样，从来没有一个固定的走向未来的道路，我们所要做的，就是寻找自己的闪光点，发挥自己的特长，走最适合自己的路。
