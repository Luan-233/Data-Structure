...
+ 普通堆的维护：
  ...
  + 例题2：洛谷P2048 [NOI2010] 超级钢琴
    ...
    考虑固定右端点的情况下左端点的范围是一个连续的区间，那么设四元组$(l,r,pos,val)$为左端点位于$[l,r]$内，右端点位于$pos$时，最大的区间和是多少，假设位置$x$满足$l\le x\le r$且区间$[x,pos]$的区间和最大，那么考虑类似于$n$路归并的过程，下一个位置要寻找次大值，显然在是左端点位于区间$[l,x-1]$或者$[x+1,r]$内的最大值，用前缀和加上一个支持查询区间最值位置的数据结构维护，所有的四元组用小根堆维护。堆内每取出一个元素，就在区间最值处分裂成两侧的区间最值再插入回来，反复操作$K$次就可以得到结果。总复杂度$O((n+k)logn)$。
...
+ 基本操作
  ...
  + 定长01序列的操作
    ...
    + 例题3：洛谷P3968 [TJOI2014] 电源插排
      ...
      在这个基础上套上动态开点即可。需要额外注意新结点的信息以及哨兵结点信息的维护。
...
+ 动态开点
  ...
  这个写法可以在线段树有用信息稀疏的情况下节省空间。同时为后面的线段树合并做铺垫。很多时候可以替代平衡树进行操作。如果对于线段树记录左右儿子的写法比较熟练，那么对动态开点应该会感到很亲切。具体而言，在线段树上dfs的时候，若一个结点不存在，则动态的申请出它的空间。
...
+ 莫队的常数优化技巧&细节
  ...
  + 细节：若四个while循环排布顺序不当，可能会导致指针$l>r$的情况，此时有可能会造成数组内存下溢（常见于值域分块）。所以优先考虑扩展两端区间，然后再考虑收缩区间。
...
T(n)=aT(\frac{n}{b})+f(n)
$$
其中$f(n)$为一个多项式有界函数，$T(n)$在$n$很小的时候为有限值，可以看做是常数。那么由递归树每一层求和得到
$$
...
+ 当$a<b^k$的时候，即$log_ba<k$，忽略若干常数的变化得到$T(n)=O(n^k\times\frac{a^{log_bn}}{n^k})=O(n^{log_ba})$，但注意到递归树根处存在一个$f(n)=O(n^k)$的复杂度，此时上式得到的复杂度是递归树除去树根之外的复杂度之和，复杂度被$O(n^k)$支配。
